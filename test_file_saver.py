...
class TestSaveFile:
    ...
    def test_over_64kb_emoji_file(self, tmp_path_file):
        ...
        This is the primary regression test for issue #62. Each emoji is
    ...
    def test_over_64kb_cjk_file(self, tmp_path_file):
        """File with CJK characters where byte count exceeds 64KB."""
    ...
    def test_boundary_split_multibyte(self, tmp_path_file):
        """Content where 64KB boundary splits a multibyte sequence."""
        # Fill up to just before 64KB with ASCII, then add multibyte
        ascii_part = "X" * (BUFFER_SIZE - 2)
        # Add a 4-byte emoji that crosses the 64KB boundary
    ...
    def test_char_count_under_64k_byte_count_over(self, tmp_path_file):
        ...
        This directly targets the root cause where the buffer was sized
        by character count instead of byte length.
        """
        # 4-byte emoji: 16385 chars = 65540 bytes (just over 64KB)
